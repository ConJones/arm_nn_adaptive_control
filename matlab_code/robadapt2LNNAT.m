...
if( t > PD_delta_t*PD_step_num)
   ...
   Kz=50*eye(Outs); Kv=20*eye(Outs); lam=5*eye(Outs); F=50*eye(L); G=50*eye(I); kappa=.1; ZB=.1;% controller parameters
...
if( t > NN_delta_t*NN_step_num && t < NN_delta_t*(NN_step_num +1) )
   T_w_dot=(F*NNOut*r'-F*(diag(NNOut)*(eye(L)-diag(NNOut)))*[ones(I,1) V]'*NNIn'*r'-kappa*F*norm(r)*W)*NN_delta_t;

   T_v_dot=(G*NNIn'*((diag(NNOut(2:end))*(eye(L-1)-diag(NNOut(2:end))))*W(2:end,:)*r)' - kappa*G*norm(r)*V)*NN_delta_t;
...
H3=-0.5*qp(1)^2*((m3+0.25*m2)*a2^2*sin(2*(q(2)+q(3))) + (m3 + m2)*a1*a2*cos(q(2))*cos(q(2)+q(3))) ...
